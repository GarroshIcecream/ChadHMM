...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(
            ...
            for keyword in ["large", "stress", "memory", "time"]
        ):
...
def financial_data():
    ...
    return {"X": X, "states": states, "lengths": [500, 500]}
...
def sequence_data():
    ...
    return {"X": X_one_hot, "X_raw": X, "lengths": lengths, "sequences": sequences}
...
def large_dataset():
    ...
    torch.manual_seed(42)
    return {"X": torch.randn(5000, 3), "lengths": [2500, 2500]}
...
def high_dimensional_data():
    ...
    torch.manual_seed(42)
    return {"X": torch.randn(1000, 10), "lengths": [500, 500]}
...
def assert_model_properties(model, n_states=None, n_features=None):
    ...
    if n_states is not None:
        assert model.n_states == n_states, (
            f"Expected {n_states} states, got {model.n_states}"
        )
    if n_features is not None:
        assert model.n_features == n_features, (
            f"Expected {n_features} features, got {model.n_features}"
        )
...
def assert_predictions_valid(predictions, lengths, n_states):
    ...
    assert len(predictions) == len(lengths), (
        "Number of predictions doesn't match number of sequences"
    )
    for i, (pred, length) in enumerate(zip(predictions, lengths, strict=False)):
        assert len(pred) == length, (
            f"Prediction {i} length {len(pred)} doesn't match expected {length}"
        )
